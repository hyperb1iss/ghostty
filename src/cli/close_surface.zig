...
/// The `close-surface` command closes a specific surface in a running
/// Ghostty instance.
///
/// Use `+list-surfaces` to discover surface IDs, then use those IDs to
/// close specific terminals.
///
/// Flags:
///
///   * `--class=<class>`: Target a custom instance of Ghostty.
///
///   * `--surface=<id>`: The surface ID to close (required).
///       Get surface IDs from `+list-surfaces`.
///
/// Examples:
///
///   Close a specific surface:
///     ghostty +close-surface --surface=0x153872000
///
...
fn runArgs(
    ...
    const success = apprt.socket.performIpc(alloc, target, .close_surface, .{
    ...
    if (!success) {
        try stderr.print("Failed to close surface\n", .{});
        return 1;
    }
